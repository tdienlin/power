...
In Germany we have the following saying: Everything that you need to know about power has already been said -- but not yet by everyone. For what it's worth, in what follows I take my own turn and try to explain the concept of statistical power using simple words, simulations of data, and some gifs.

Note that this post is written primarily for students in order to provide some guidelines for empirical theses or reports. I will provide the R-code necessary for all the analyses, and you can also download everything from my [github](https://github.com/tdienlin/power). 
...
In short, it describes the probabilility of being able to cry "Eureka! I found it!", when in fact there was something that could be found. In other words, if there is an effect, how often will you find it in the long run? Often, though not always, finding an effect is exactly what you want, which is why ideally you would want to increase that probability. 
...
How many? Good question. 200 seems quite a lot -- that should do the job, right? So from the population, we will now draw a sample of 200 students from the population and estimate the correlation. Will we find an effect ... ?
...
On the basis of the results we could not reject the privacy paradox. We would even feel tempted to conclude that indeed, the behavior of people seems somewhat paradoxical. 

(Bear in mind that a nonsignificant result in frequentist null hypothesis testing, which is what we have done here, can never actually support the null hypothesis (in this case, that the relationship is zero). To draw such an inference, we would need to use a different approach; e.g., equivalence testing or Bayesian statistics.) 

However, remember that in the population there is an actual effect! So what went wrong? Bad luck? 

Let us run another study. Again, we draw a random sample of 200 students from our population. Maybe this time we get lucky?
...
Indeed, we find a correlation of _r_ = `r results_complete$estimate` and a p-value of _p_ = `r results_complete$p.value`, which is significant. Hooray! Drawing another sample (with the same sample size) thus gives us a different result. 
...
What do we see? On the x-axis, you see the number of each study (1 - 100), whereas the y-axis shows the effect size we found for each study. The blue dots represent significant effects, the red dots represent nonsignificant ones. 
...
If we want to analyze the privacy paradox, we would more often than not claim that there is no effect, that the privacy paradox does exis exist, when in fact it does not. In other words, with 200 people we simply cannot analyze the privacy paradox! Our study would not be informative, it would not add _anything_ to our understanding of the theoretical problem.
...
However, instead of basing your power analysis on a meta-analysis/meta-review (whose effect is likely to be inflated anyway), it is _much_ more useful to determine a so-called smallest effect size of interest (SESOI) (e.g., Lakens et al., 2018). In other words, you want to define an effect size that you think is already large enough to qualify as support for your theoretical assumption.
...
But what do we do if we do have little resources? Fortunately, there are several valid options:
...
Thanks go out to Niklas, Philipp, Romy, and Sabine who all provided valuable feedback to this blog post! Thank you very much! 
In general, I'm always happy to get feedback. So if you've spotted something that's false or if you have further recommendations, please let me know -- I'm here to learn!
...
- Button, K. S., Ioannidis, J. P. A., Mokrysz, C., Nosek, B. A., Flint, J., Robinson, E. S. J., & Munafò, M. R. (2013). Power failure: Why small sample size undermines the reliability of neuroscience. _Nature Reviews. Neuroscience_, 14(5), 365–376. https://doi.org/10.1038/nrn3475
